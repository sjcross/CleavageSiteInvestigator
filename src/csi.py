...
if write_heatmap_csv_auto:
    ...
    heatmap_writer.write(root_name+'_autoheatmap.csv', freq_full, None, None, append_dt, commandStr=commandStr)

if write_heatmap_csv_full:
    output = True
    # Showing events as heatmap
    heatmap_writer = hmwc.HeatMapWriterCSV(sum_show=False)
    heatmap_writer.write(root_name+'_fullheatmap.csv', freq_full, reference, None, append_dt=append_dt, commandStr=commandStr)
...
if write_individual:
    output = True
    csv_writer.write_individual(root_name, results, reference, commandStr)

if write_summary:
    output = True
    csv_writer.write_summary(root_name, freq_full, reference, error_count, commandStr)
