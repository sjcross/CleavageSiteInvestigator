...
required.add_argument(
    ...
    required=True,
    help=
    "path to cassette sequence file.  This is the sequence of the cassette inserted at the cleavage site."
)
...
required.add_argument(
    ...
    required=True,
    help=
    "path to reference sequence file.  This is the sequence which has been digested."
)
...
required.add_argument(
    ...
    required=True,
    help=
    "path to test sequence file.  This is the sequence of the cleaved sample with cassette inserted."
)
...
optional.add_argument(
    ...
    default="",
    help=
    "expression defining filter for accepted number of repeats.  Uses standard Python math notation, where 'x' is the number of repeats (e.g. 'x>=3' will process all sequences with at least 3 repeats)"
)
...
optional.add_argument(
    ...
    default=0,
    help=
    "number of additional nucleotides to be displayed either side of the cleavage site"
)
...
optional.add_argument(
    ...
    default=1,
    help=
    "half width of the local sequences to be extracted at restriction sites")
...
optional.add_argument(
    ...
    type=int,
    default=10,
    help="maximum number of bp between 3' and 5' restriction sites")

optional.add_argument("-mq",
                      "--min_quality",
                      type=float,
                      default=1.0,
                      help="minimum match quality (\"1\" is perfect)")

optional.add_argument("-nb",
                      "--num_bases",
                      type=int,
                      default=20,
                      help="number of bases to match")

optional.add_argument("-sr",
                      "--show_results",
                      action='store_true',
                      help="display results in terminal as they are generated")

optional.add_argument(
    "-sp",
    "--show_plots",
    action='store_true',
    help="display plots in pyplot windows as they are generated")

optional.add_argument("-v",
                      "--verbose",
                      action='store_true',
                      help="display detailed messages during execution")
...
searcher = su.SequenceSearcher(aligner,
                               max_gap=max_gap,
...
freq_local = ru.get_local_sequence_frequency(results, ru.StrandMode.BOTH,
                                             ru.LocalMode.BOTH, local_r)
freq_5p = ru.get_local_sequence_frequency(results, ru.StrandMode.BOTH,
                                          ru.LocalMode.FIVE_P, local_r)
freq_3p = ru.get_local_sequence_frequency(results, ru.StrandMode.BOTH,
                                          ru.LocalMode.THREE_P, local_r)

if show_results:
    print("\rRESULTS:")
    print("    Full sequence frequency:\n")
    ru.print_full_sequence_frequency(ref,
                                     freq_full,
                                     extra_nt=extra_nt,
                                     offset="")

    print("    Local dinucleotide frequencies:\n")
    ru.print_local_sequence_frequency(freq_local,
                                      nonzero_only=False,
                                      offset="")
